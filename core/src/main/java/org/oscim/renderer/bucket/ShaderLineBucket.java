...
public class ShaderLineBucket extends DefaultBucket {
    static final Logger log = LoggerFactory.getLogger(ShaderLineBucket.class);

    public LineStyle line;

    @Override
    public RenderBucket getNewInstance(int level) {
        return new ShaderLineBucket(level);
    }

    @Override
    public int getVertexShortCnt() {
        return 2;
    }

    @Override
    public RenderBucket draw(RenderBucket b, GLViewport v, float div, RenderBuckets buckets) {
        return Renderer.draw(b,v);
    }

    public ShaderLineBucket(int level) {
        super(true, false);
        this.level = level;
    }
    ...
    public static class Renderer {
        ...
        public static RenderBucket draw(RenderBucket l, GLViewport v) {
            ...
            for (; l != null && l.type == HAIRLINE; l = l.next) {
                ShaderLineBucket ll = (ShaderLineBucket) l;
